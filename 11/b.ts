...
// mod factor to keep numbers small
const modFactor = parsed.reduce((a, b) => a * parseInt(b.test[2]), 1);
...
for (let i = 0; i < 10000; i++) {
    parsed.forEach((monkey, idx) => {
        while (monkey.startingItems.length) {
            ...
            const evalString = `${item} ${monkey.op[3]} ${monkey.op[4] === "old" ? item : monkey.op[4]
                }`;
            const res = eval(evalString) % modFactor;
